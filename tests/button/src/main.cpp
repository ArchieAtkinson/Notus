...
ZTEST(button, test_on_release) // NOLINT
{
    ...
    const int on_release_min_time_ms = 100;
    button.add_on_release_callback(cb_test2, std::chrono::milliseconds(on_release_min_time_ms));

    gpio_emul_input_set(irq_pin.port, irq_pin.pin, 1);
    k_sleep(K_MSEC(on_release_min_time_ms));
