...
bool print_channel_data_iterator(const struct zbus_channel *chan, void *user_data)
{
...
bool print_observer_data_iterator(const struct zbus_observer *obs, void *user_data)
{
	auto *count = static_cast<int*>(user_data);
...
void listener_callback(const struct zbus_channel *chan)
{
    ...
    {
		const auto *msg = static_cast<const module1::module1_out_msgs*>(zbus_chan_const_msg(chan)); 
		LOG_INF("%s", std::get<0>(*msg).value.c_str());
    }
}
